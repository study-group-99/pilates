...
TEST(TestFtMemcpy, BasicReturn) {
    ...
    s = "Some other random text for s3";
	want = memcpy(s1, s, 30);
    got = ft_memcpy(s2, s, 30);
    EXPECT_STREQ((char *)got, (char *)want) << "Input: char s2[49] = \"0\"; char *s = \"Some other random text for s3\"; ft_memcpy(s2, s, 30);";
}
...
TEST(TestFtMemcpy, DoubleNull) {
	void *got = ft_memcpy(NULL, NULL, 0);
	EXPECT_EQ(NULL, got) << "Input: void *got = ft_memcpy(NULL, NULL, 0);";
	
	got = ft_memcpy(NULL, NULL, 5);
	EXPECT_EQ(NULL, got) << "Input: void *got = ft_memcpy(NULL, NULL, 5);";
}